...
aistat switch --if-needed              # only switch when the active account crosses a threshold, else "no switch needed"
aistat switch --if-needed --notify     # same, plus a desktop notification on switch or on a hit with no better account (macOS)
...
- [internal/autoswitch/](internal/autoswitch/) — pure threshold resolution for `switch --if-needed`: `Resolve(getenv)` reads `AISTAT_IF_ABOVE_5H` / `AISTAT_IF_ABOVE_WEEKLY` (non-empty process env > built-in default 85/95), `ParseThreshold` accepts an integer 1-100 or `"off"` to disable a window.
